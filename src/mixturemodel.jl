function CalibrationErrors.unsafe_skce_eval_targets(
    κtargets::Kernel,
    p::AbstractMixtureModel,
    y,
    p̃::AbstractMixtureModel,
    ỹ
)
    ...
    s = probsi * p̃_probs[1] * CalibrationErrors.unsafe_skce_eval_targets(
        κtargets, componentsi, y, p̃_components[1], ỹ
    )
    for j in 2:length(p̃_components)
        s += probsi * p̃_probs[j] * CalibrationErrors.unsafe_skce_eval_targets(
